...
class Pipeline:
    def __init__(
        ...
        if model_type == ModelTypes.VISUAL:
            ...
            self.train_dataset = InertialDataset(train_dir, sequence_length=sequence_length)
            self.val_dataset = InertialDataset(val_dir, sequence_length=sequence_length)
            self.model = InertialModel(lstm_hidden_size=lstm_hidden)
