...
# --- Camera ---
CAMERA_YAW_DEG = 0.0  # yaw offset in degrees (applied on top of heading)
# "tangent": yaw follows trajectory tangent (continuous rotational motion)
# "fixed": constant yaw for every frame
HEADING_MODE = os.environ.get("DRONE_HEADING_MODE", "tangent").strip().lower()
...
RENDER_IMAGES = False  # False = export poses only (much faster)
...
def resample_polyline_constant_speed(points, speed, fps):
    """
    Resample a polyline at constant speed and fixed sample rate.
    """
    if len(points) < 2:
        return points
    if speed <= 0.0:
    ...
    n_steps = max(2, int(round(total_time * fps)))
    target_s = [total_len * (k / n_steps) for k in range(n_steps + 1)]

    out = []
    seg_idx = 0
    for s in target_s:
        while seg_idx < len(lengths) - 2 and lengths[seg_idx + 1] < s:
            seg_idx += 1

        s0 = lengths[seg_idx]
        s1 = lengths[seg_idx + 1]
        p0 = points[seg_idx]
        p1 = points[seg_idx + 1]
        if s1 - s0 < 1e-12:
            out.append(p0.copy())
...
def sample_circle_positions(radius, height, n_laps, speed, fps):
    """
    Analytic circle sampled directly in time for smooth continuous motion.
    """
    if radius <= 0.0:
        raise ValueError("Circle radius must be > 0")
    if speed <= 0.0:
    ...
    total_len = 2.0 * math.pi * radius * n_laps
    total_time = total_len / speed
    n_steps = max(2, int(round(total_time * fps)))
    omega = speed / radius
    ...
    for k in range(n_steps + 1):
        ...
        positions.append(Vector((x, y, height)))
    return positions


def sample_figure8_positions(width, length, height, n_laps, speed, fps):
    """
    Smooth figure-8 sampled from parametric curve then arc-length resampled
    to maintain near-constant speed.
    """
    a = width / 2.0
    b = length / 2.0

    samples_per_lap_dense = 2000
    dense = []
    total_samples = samples_per_lap_dense * n_laps
    for i in range(total_samples + 1):
        u = i / total_samples
        t = 2.0 * math.pi * n_laps * u
        x = a * math.sin(t)
        y = b * math.sin(t) * math.cos(t)
        dense.append(Vector((x, y, height)))

    return resample_polyline_constant_speed(dense, speed, fps)


def compute_tangent_yaws(positions, yaw_offset_rad=0.0):
    """
    Estimate heading from trajectory tangent with finite differences.
    """
    if not positions:
        return []
...
def build_positions(shape_name, common_cfg, shape_cfg):
    """
    Build positions with smooth continuous motion where possible.
    """
    waypoints = build_path_waypoints(shape_name, common_cfg, shape_cfg)

    if shape_name == "circle":
        positions = sample_circle_positions(
            shape_cfg["radius"],
            common_cfg["height"],
            common_cfg["laps"],
            common_cfg["speed"],
        ...
        positions = sample_figure8_positions(
            shape_cfg["width"],
            shape_cfg["length"],
            common_cfg["height"],
            common_cfg["laps"],
        ...
        # Square keeps the intended cornering behavior.
...
def setup_keyframes(cam_obj, positions, yaw_values):
    """
    ...
    for idx, pos in enumerate(positions):
        ...
        if isinstance(yaw_values, (list, tuple)):
            yaw_rad = yaw_values[idx]
        else:
            yaw_rad = yaw_values
...
def main():
    ...
    # ── Trajectory ──────────────────────────────────────────
    waypoints, positions = build_positions(shape_name, common_cfg, shape_cfg)
    ...
    if HEADING_MODE not in {"fixed", "tangent"}:
        raise ValueError(
            f"DRONE_HEADING_MODE='{HEADING_MODE}' is invalid. "
            "Choose one of ['fixed', 'tangent']"
        )
    if HEADING_MODE == "tangent":
        yaw_values = compute_tangent_yaws(positions, math.radians(CAMERA_YAW_DEG))
    else:
        yaw_values = math.radians(CAMERA_YAW_DEG)
    meta["heading_mode"] = HEADING_MODE

    # ── Keyframes ───────────────────────────────────────────
    setup_keyframes(cam, positions, yaw_values)
