...
class SequenceEvaluator:
    ...
    def compute_metrics(self, prediction: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        for i in range(len(gt_pos) - 1):
            ...
            gt_dq = self._quaternion_multiply(
                self._quaternion_inverse(gt_quat[i]),
                gt_quat[i + 1]
            )
            ...
            est_dq = self._quaternion_multiply(
                self._quaternion_inverse(est_quat[i]),
                est_quat[i + 1]
            )
    ...
    @staticmethod
    def _align_se3(est: np.ndarray, gt: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def visualize(
        ...
        ax3.axvline(metrics['ate_mean'], color='red', linestyle='--', 
                   linewidth=2, label=f"Mean: {metrics['ate_mean']:.3f}m")
        ax3.axvline(metrics['ate_median'], color='green', linestyle='--',
                   linewidth=2, label=f"Median: {metrics['ate_median']:.3f}m")
    ...
    def save_outputs(
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['frame', 'gt_tx', 'gt_ty', 'gt_tz', 'gt_qw', 'gt_qx', 'gt_qy', 'gt_qz',
            ...
            for i in range(len(prediction['abs_positions'])):
                ...
                writer.writerow([
                    ...
                    est_quat[0], est_quat[1], est_quat[2], est_quat[3],
                ])
        ...
        with open(tum_gt_path, 'w') as f:
            for i in range(len(self.gt_poses['positions'])):
                ...
                f.write(f"{frame} {pos[0]:.6f} {pos[1]:.6f} {pos[2]:.6f} "
...
def test_single_sequence(
    ...
    print(f"    RMSE X/Y/Z: {metrics['rmse_x']:.4f} / {metrics['rmse_y']:.4f} / {metrics['rmse_z']:.4f} m")
    print(f"    RPE Trans: {metrics['rpe_trans_rmse']:.4f} m")
    print(f"    RPE Rot:   {metrics['rpe_rot_rmse']:.4f} rad ({np.rad2deg(metrics['rpe_rot_rmse']):.2f}°)")
...
def parse_args():
    ...
    type_group.add_argument('-vi', action='store_true', help='Test Visual-Inertial Model')
