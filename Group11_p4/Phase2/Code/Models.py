...
class VisualModel(nn.Module):
    """
    CNN model for visual odometry using adjacent frame pairs.
    - each sample is an independent (t, t+1) image pair.

    Output: relative pose (batch_size, 6) = [dx, dy, qw, qx, qy, qz]
    """
    ...
    def __init__(
        ...
        # compress feature maps to a compact vector per image
        ...
        # regression head for relative pose prediction
...
class VisualInertialModel(nn.Module):
    ...
    Architecture:
    - Visual branch: CNN -> features (2*feature_size)
    ...
    def __init__(
        ...
        # Visual branch: CNN from VisualModel
        self.cnn = self._build_cnn_layers()
        self.feature_bottleneck = nn.Sequential(
            nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(256),
    ...
    def forward(self, batch):
        ...
        # attempt to force model to use both modalities
